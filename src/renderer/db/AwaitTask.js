...
export default {
  QueryAwaitTaskInfo: async function(ajid) {
    ...
    try {
      ...
      let sql = `SELECT * FROM  mz_awaittask awaitTask  WHERE  awaitTask.ajid = '${ajid}'`;
  ...
  QueryHaveSameInfo: async function(
    ...
    try {
      ...
      if (task_type === "1" || task_type === "2") {
        sql = `SELECT COUNT(*)::int count FROM mz_awaittask WHERE zz_code='${zz_code}'and zz_name='${zz_name}' and jyhm='${jyhm}' and task_type='${task_type}' `;
      } else {
        sql = `SELECT COUNT(*)::int count FROM mz_awaittask WHERE yh_name='${yh_name}'and cxzh='${cxzh}' and zh_ztlb_name='${zh_ztlb_name}' and task_type='账卡号'`;
      }
      console.log(sql);
      const res = await client.query(sql);
  ...
  DeleteAwaitTask: async function(ajid, ids) {
    ...
    try {
      await Cases.SwitchCase(client, ajid);
      let sql;
      sql = `DELETE FROM mz_awaittask WHERE ID in (${ids})`;
      console.log(sql);
      const res = await client.query(sql);
      return { success: true };
    } finally {
  ...
  InsertNewAwaitTask: async function(
    ...
    let sql;
    if (task_type === "1") {
      sql = ` INSERT INTO mz_awaittask(ajid, yh_name, zz_code, zz_name, zh_ztlb, zh_ztlb_name, tjsj, tjrw, task_type, type_id, jyhm, target, xgsj) 
      VALUES(
      '${ajid}',
      '请选择银行...', 
      '${zz_code}', 
      ...
      sql = `INSERT INTO mz_awaittask(ajid, cxzh, yh_code, yh_name,zh_ztlb, zh_ztlb_name, tjsj, tjrw, task_type, type_id, jyhm, target, xgsj) 
      VALUES(
        '${ajid}',
        '${cxzh}',  
        '',  
  ...
  UpdateAwaitTaskById: async function(
    ...
    let sql;
    if (task_type === "1") {
      sql = `UPDATE mz_awaittask SET zz_code = '${zz_code}',zz_name = '${zz_name}',tjrw = '00000000', jyhm = '${jyhm}', xgsj = '${new Date().Format(
        "yyyy-MM-dd hh:mm:ss"
      )}' WHERE id = ${id};`;
    } else if (task_type === "2") {
      sql = `UPDATE mz_awaittask SET zz_code = '${zz_code}',zz_name = '${zz_name}',tjrw = '00000000', jyhm = '${jyhm}', xgsj = '${new Date().Format(
  ...
  QueryHaveSameInfoInModelTable: async function(ajid, cxzh) {
    let sql = `SELECT COUNT(*)::int count FROM mz_awaittask
