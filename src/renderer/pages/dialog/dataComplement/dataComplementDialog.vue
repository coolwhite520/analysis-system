<template>
  <el-dialog v-dialogDrag :close-on-click-modal="false" class="standard-data-dialog" :append-to-body="true"
    :visible="showDataComplementVisible" width="50%" @close="handleClose" :modal="true">
    ...
    <div v-loading="loading" :element-loading-text="loadingText">
      <el-row>
        <el-col :span="12" style="text-align: left; font-size: 10px">
          可补数据：<span style="color: #d15e27"><b>{{ num }}</b>
          </span>
          &nbsp; &nbsp; &nbsp; &nbsp;待补空数据：<span style="color: green"><b>{{ count }}</b></span>
        </el-col>
        <el-col :span="12" style="text-align: right">
          <el-switch style="display: block" v-model="switchValue" active-color="#13ce66" inactive-color="#ff4949"
            inactive-text="按已调单补全" active-text="全部补全" @change="handleChangeSwitch">
          </el-switch>
        </el-col>
      </el-row>
      <el-row style="margin-top: 20px">
        <el-table ref="multipleTable" style="width: 100%" :data="dataShowRows" size="mini" :max-height="300"
          :height="300" stripe border>
          <el-table-column fixed type="index" label="编号" header-align="center" align="center"></el-table-column>
          <el-table-column header-align="center" align="center" label="卡号（主账号）" prop="Zh" show-overflow-tooltip>
          </el-table-column>
          <el-table-column label="户名" show-overflow-tooltip header-align="center" align="center">
            <template slot-scope="scope">
              <div v-if="scope.row.ZhmcIsenable">
                <el-select :value="scope.row.Zhmc" placeholder="请选择" size="mini" @change="
                  (val) => {
                    handleChangeDataZhmc(val, scope.$index);
                  }
                ">
                  <el-option v-for="(item, index) in scope.row.ZhmcList" :key="index" :label="item" :value="item">
                  </el-option>
                </el-select>
              </div>
          ...
          <el-table-column label="开户银行" show-overflow-tooltip header-align="center" align="center">
            <template slot-scope="scope">
              <div v-if="scope.row.KhyhIsenable">
                <el-select :value="scope.row.Khyh" placeholder="请选择" size="mini" @change="
                  (val) => {
                    handleChangeDataKhyh(val, scope.$index);
                  }
                ">
                  <el-option v-for="(item, index) in scope.row.KhyhList" :key="index" :label="item" :value="item">
                  </el-option>
                </el-select>
              </div>
              <div v-else style="color: gray">
          ...
          <el-table-column label="证件号码" show-overflow-tooltip header-align="center" align="center">
            <template slot-scope="scope">
              <div v-if="scope.row.ZzhmIsenable">
                <el-select :value="scope.row.Zzhm" placeholder="请选择" size="mini" @change="
                  (val) => {
                    handleChangeDataZzhm(val, scope.$index);
                  }
                ">
                  <el-option v-for="(item, index) in scope.row.ZzhmList" :key="index" :label="item" :value="item">
                  </el-option>
                </el-select>
              </div>
              <div v-else style="color: gray">
        ...
        <div style="margin-top: 10px; text-align: right">
          <el-pagination small @size-change="handleSizeChange" @current-change="handleCurrentChange"
            :current-page="currentPage4" :page-sizes="[50, 100, 150, 200, 300]" :page-size="pageSize"
            layout="total, sizes, prev, pager, next, jumper" :total="this.dataRows.length">
          </el-pagination>
        </div>
      </el-row>
      <el-row style="text-align: center; margin-top: 20px">
        <el-button @click="handleClose" style="width: 35%" size="medium">取消</el-button>
        <el-button @click="handleClickSure" :disabled="count === 0" type="primary" style="width: 35%" size="medium">确定补全
        </el-button>
      </el-row>
    </div>
