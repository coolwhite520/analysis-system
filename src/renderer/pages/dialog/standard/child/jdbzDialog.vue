<template>
  <el-dialog v-dialogDrag :close-on-click-modal="false" class="standard-data-dialog" :append-to-body="true"
    :visible="showJdbzDialogVisible" width="30%" @close="handleClose" :modal="true">
    ...
    <div>
      <span style="font-size: 10px; color: #30693d; margin-bottom: 20px">提示：{{ this.formData.tips }}，如不确定输入内容，</span>
      <el-button type="text" size="mini" @click="handleClickOpenFile">打开文件查看</el-button>
    </div>
    <el-form :model="formData" size="mini" label-width="80px" :rules="formRules" ref="formRule">
      <el-form-item label="【进】：" prop="inFlag">
        <el-input v-model="formData.inFlag" placeholder="请输入文件中含义为【进帐】的文字内容"></el-input>
      </el-form-item>
      <el-form-item label="【出】：" prop="outFlag">
        <el-input v-model="formData.outFlag" placeholder="请输入文件中含义为【出帐】的文字内容"></el-input>
      </el-form-item>
    </el-form>
    <el-row style="text-align: center">
      <el-button type="primary" @click="onSubmit('formRule')" style="width: 90%">保存设置</el-button>
    </el-row>
  </el-dialog>
</template>
<script>
