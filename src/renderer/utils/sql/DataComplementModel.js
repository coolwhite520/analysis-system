...
function DataSupplementWinModel(ajid) {
  ...
  this.createTableIndex = async function() {
    ...
    try {
      ...
      ON mz_bank_records (cxkh, jydfzkh);`;
  ...
  FROM
    ...
    FROM
      ...
      A INNER JOIN mz_account_info B   ON A.zh = B.kh 
    WHERE
      B.ajid = ${this.ajid} 
      AND  (
         (
      ...
      return await new Promise(async (resolve, reject) => {
        ...
        for (let current of this.AccountModelscan) {
          ...
          if (this.dictionary_2.hasOwnProperty(key)) {
            let text4 = "update mz_bank_records set  ";
            let text5 = "";
          ...
          key = current.Zh + "_0";
          if (this.dictionary_2.hasOwnProperty(key)) {
            let text6 = "update   mz_bank_records set  ";
            let text7 = "";
            if (current.IsZZHMHandUpdate()) {
              let selectzzhm = current.get_ZzhmSelected();
              if (!Default.IsNullOrEmpty(selectzzhm)) {
  ...
  this.execSqlUpdate = async function() {
    ...
    let sql2 = `UPDATE mz_bank_records A 
    SET JYDFZJHM =
    CASE
    ...
    let sql3 = `UPDATE mz_bank_records 
      SET JYZJHM =
      CASE
